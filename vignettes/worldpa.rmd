...
It allows users to download spatial layers of terrestrial and marine protected areas (PA) for world countries using the WDPA API ([https://api.protectedplanet.net](https://api.protectedplanet.net)).
...
The returned object is a **sf object** (**GEOMETRY**) of protected areas defined 
in the EPSG 4326. Type `?get_wdpa` to get more information about the attributes. 
The function `get_wdpa()` has also exported the protected areas as a **geopackage**
(`.gpkg`). User can import this layer with the function `sf::st_read()`.
